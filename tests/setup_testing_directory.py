...
def get_testing_directory() -> str:
    ...
    raise ValueError(
        "please run setup_testing_directory.py before attempting to run unit tests"
    )


def setup_testing_directory(
    datadir: Union[str, os.PathLike], overwrite: bool = False
) -> str:
    testing_path_file = "testing_directory.txt"

    should_setup = True
    if os.path.isfile(testing_path_file):
        with open(testing_path_file) as f:
            testing_directory = f.read()
            if not os.path.isfile(testing_directory):
                raise ValueError("saved testing directory {} does not exist, re-run ")
                warnings.warn(
                    f"Saved testing directory {testing_directory} does not exist, downloading Thumos14...",
                    stacklevel=2,
                )
            else:
                should_setup = False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Setting up image directory for opencv transforms testing"
    )
    parser.add_argument(
        "-d", "--datadir", default=os.getcwd(), help="Imagenet directory"
    )
